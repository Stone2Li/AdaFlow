...
def generate_samples(config, i, iteration=50, n_samples=64, seq_len=1000, input_dim=1):
    ...
    with torch.no_grad():
        ...
        for i in range(iteration):
            ...
            p_new = p_new + p_delta.unsqueeze(2)
        return p_new, labels
...
def apply_flow_matching(config,data,label,iterations,top_num=50):
    ...
    for i in range(iterations): #样本生成的迭代次数
        feature_scores=calculate_featrue_score(p_0, label_0)
        ...
        train_flow_matching(config,p_0,label_0,i,iterations)
        ...
        p_0=torch.cat([p_0,p_new],dim=0)
        label_0=torch.cat([label_0,label_new])
        print(f"[{i+1}/{iterations}] Wasserstein_0:{wd_0:.4f} Wasserstein_1:{wd_1:.4f}")
    return all_feature_scores, all_top_features, all_wd_0, all_wd_1
...
if __name__=="__main__":
    ...
    all_feature_scores,all_top_features,all_wd_0,all_wd_1 = apply_flow_matching(config, data, label, iteration)
    results = {
