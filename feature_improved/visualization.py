...
def visualize_data_distribution_1(p_0, p_new, label_0, label_new, iterations, save_path):
    """可视化生成样本呢和初始样本的散点图"""
    fig, axes = plt.subplots(2, 5, figsize=(20, 8), sharex=True, sharey=True)
    fig.suptitle("Data Distribution Across Iterations", fontsize=16)
    p_0_flat = p_0.cpu().numpy().reshape(p_0.shape[0], -1)  # [n_samples, seq_len * input_dim]
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(f"{save_path}/Data Distribution across Iterations_1.png")
