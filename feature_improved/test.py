...
def visualize_data_distribution(p_0, p_new, label_0, label_new, iterations, save_path):
    ...
    for i, ax in enumerate(axes.flat):
        if i < iterations:
            ...
            p_new_flat = p_new_flat[:, :]
            ax.scatter(p_0_flat[label_0_np==0, 0], p_0_flat[label_0_np==0, 1], c='blue', label='Old Health', alpha=0.5)
            ax.scatter(p_0_flat[label_0_np==1, 0], p_0_flat[label_0_np==1, 1], c='red', label='Old Disease', alpha=0.5)
            ax.scatter(p_new_flat[label_new_np==0, 0], p_new_flat[label_new_np==0, 1], c='lightblue', label='New Health', alpha=0.5)
            ax.scatter(p_new_flat[label_new_np==1, 0], p_new_flat[label_new_np==1, 1], c='pink', label='New Disease', alpha=0.5)
    ...
    plt.savefig("./Data Distribution across Iterations.png")
